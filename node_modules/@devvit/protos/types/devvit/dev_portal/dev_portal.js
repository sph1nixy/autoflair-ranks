...
export const BotRegistrationServiceName = "devvit.dev_portal.BotRegistration";
export class BotRegistrationClientImpl {
    constructor(rpc, opts) {
        this.service = opts?.service || BotRegistrationServiceName;
        this.rpc = rpc;
        this.AgreeToBounty = this.AgreeToBounty.bind(this);
    }
    AgreeToBounty(request, metadata) {
        const data = AgreeToBountyRequest.encode(request).finish();
        const promise = this.rpc.request(this.service, "AgreeToBounty", data, metadata);
        return promise.then((data) => AgreeToBountyResponse.decode(_m0.Reader.create(data)));
    }
}
export const BotRegistrationDefinition = {
    name: "BotRegistration",
    fullName: "devvit.dev_portal.BotRegistration",
    methods: {
        /** Update a registration to agree to the current bounty terms, logging at what time they agreed. */
        agreeToBounty: {
            name: "AgreeToBounty",
            requestType: AgreeToBountyRequest,
            requestStream: false,
            responseType: AgreeToBountyResponse,
            responseStream: false,
            options: {},
        },
    },
