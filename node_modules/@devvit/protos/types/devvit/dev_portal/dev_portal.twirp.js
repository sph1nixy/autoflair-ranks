...
export var BotRegistrationMethod;
(function (BotRegistrationMethod) {
    BotRegistrationMethod["AgreeToBounty"] = "AgreeToBounty";
})(BotRegistrationMethod || (BotRegistrationMethod = {}));
export const BotRegistrationMethodList = [BotRegistrationMethod.AgreeToBounty];
export function createBotRegistrationServer(service) {
    return new TwirpServer({
        service,
        packageName: 'devvit.dev_portal',
        serviceName: 'BotRegistration',
        methodList: BotRegistrationMethodList,
        matchRoute: matchBotRegistrationRoute,
    });
}
function matchBotRegistrationRoute(method, events) {
    switch (method) {
        case 'AgreeToBounty':
            return async (ctx, service, data, interceptors) => {
                ctx = { ...ctx, methodName: 'AgreeToBounty' };
                await events.onMatch(ctx);
                return handleBotRegistrationAgreeToBountyRequest(ctx, service, data, interceptors);
            };
        default:
            events.onNotFound();
            const msg = `no handler found`;
...
function handleBotRegistrationAgreeToBountyRequest(ctx, service, data, interceptors) {
    switch (ctx.contentType) {
        case TwirpContentType.JSON:
            return handleBotRegistrationAgreeToBountyJSON(ctx, service, data, interceptors);
        case TwirpContentType.Protobuf:
            return handleBotRegistrationAgreeToBountyProtobuf(ctx, service, data, interceptors);
        default:
            const msg = 'unexpected Content-Type';
            throw new TwirpError(TwirpErrorCode.BadRoute, msg);
    }
}
async function handleBotRegistrationAgreeToBountyJSON(ctx, service, data, interceptors) {
    let request;
    let response;
    try {
        const body = JSON.parse(data.toString() || '{}');
        request = AgreeToBountyRequest.fromJSON(body);
    }
    catch (e) {
        if (e instanceof Error) {
            const msg = 'the json request could not be decoded';
    ...
    if (interceptors && interceptors.length > 0) {
        const interceptor = chainInterceptors(...interceptors);
        response = await interceptor(ctx, request, (ctx, inputReq) => {
            return service.AgreeToBounty(ctx, inputReq);
        });
    }
    else {
        response = await service.AgreeToBounty(ctx, request);
    }
    return JSON.stringify(AgreeToBountyResponse.toJSON(response));
}
async function handleBotRegistrationAgreeToBountyProtobuf(ctx, service, data, interceptors) {
    let request;
    let response;
    try {
        request = AgreeToBountyRequest.decode(data);
    }
    catch (e) {
        if (e instanceof Error) {
            const msg = 'the protobuf request could not be decoded';
    ...
    return Buffer.from(AgreeToBountyResponse.encode(response).finish());
}
